...
#pragma warning restore 0618
            ...
            if (subObjectFields.Any())
            {
                ...
                foreach (FieldInfo subObjectField in subObjectFields)
                {
                    ...
                    if(subObjectField.IsUnityNull() || value.IsUnityNull()){
                        continue;
                    }
                    if(value is Unity.Collections.NativeArray<int>)
                    {
                        continue;
                    }
                    //Debug.Log($"{subObjectField.Name} type={value.GetType()} {FieldPath(subObjectPath, subObjectField.Name)}");
                    ...
                    if (value != null && !parents.Contains(value))
                    {
                        if (value is System.Collections.IEnumerable)
                        {
                            ...
                            foreach (object item in value as System.Collections.IEnumerable)
                            {
                                ...
                                try{
                                    tasks = GetGenericUpdateTasks(item, FieldPath(subObjectPath, subObjectField.Name, index), parents);
                                }
                                catch(Exception){
                                    tasks = new Task[0]{};
                                }
